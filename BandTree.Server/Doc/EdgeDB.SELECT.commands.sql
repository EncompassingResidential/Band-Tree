...
START TRANSACTION;
SELECT '  Start of script date and time  ' ++ <str>std::datetime_current();
SELECT "SELECT MusicCompanyLabel { name, location, foundingdate, countryoforigin, wikipediapageid, timestamp, id }  LIMIT 10;";
SELECT MusicCompanyLabel { name, location, foundingdate, countryoforigin, wikipediapageid, timestamp, id }  LIMIT 10;
SELECT "SELECT Artist { artistname, birthdate, othernames, genres, labels, familyname, wikipediapageid, timestamp, id }  LIMIT 10;";
SELECT Artist { artistname, birthdate, othernames, genres, labels, familyname, wikipediapageid, timestamp, id }  LIMIT 10;
SELECT "SELECT Genre { name, wikipediapageid, timestamp, id } LIMIT 10;";
SELECT Genre { name, wikipediapageid, timestamp, id }  LIMIT 10;
SELECT "SELECT BandModel { title, currentmembers, pastmembers, genres, labels, wikipediapageid, wikipediapagetimestamp, id } LIMIT 10;";
SELECT BandModel { title, currentmembers, pastmembers, genres, labels, wikipediapageid, wikipediapagetimestamp, id } LIMIT 10;

# this ran ++ std::to_str(std::datetime_current())
# but it didn't change the format, still is 2024-01-07T05:59:55.658673+00:00
...
# $ date; cat ./BandTree.Server/Doc/EdgeDB.SELECT.commands.sql | edgedb ; date
# Sun, Jan  7, 2024  5:19:54 PM
