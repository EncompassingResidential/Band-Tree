# if you pipe this script into edgedb then they can all use the same DB connection.
# EdgeDB ChatGPT helper called this an EdgeQL script.
#
# This Entire script took 3 seconds VS 10 seconds 
#    for BASH running the same 4 commands with edgedb query "SELECT ....;" commands

# you have to make sure you're connecting to the right database before running these.'
# one example from command line:
# edgedb -d Band_Tree_DB

START TRANSACTION;
SELECT MusicCompanyLabel { name, location, foundingdate, countryoforigin, wikipediapageid, timestamp, id } ;
SELECT Artist { artistname, birthdate, othernames, genres, labels, familyname, wikipediapageid, timestamp, id } ;
SELECT Genre { name, wikipediapageid, timestamp, id } ;
SELECT BandModel { title, currentmembers, pastmembers, genres, labels, wikipediapageid, wikipediapagetimestamp, id } ;
SELECT BandModel { title, currentmembers, pastmembers, genres, labels, wikipediapageid, wikipediapagetimestamp, id } WHERE wikipediapageid = 124802;
COMMIT;

# $ date; cat ./BandTree.Server/Doc/EdgeDB.SELECT.commands.sql | edgedb ; date
# Sun, Jan  7, 2024  5:19:54 PM
# Connecting to EdgeDB instance 'Band_Tree_DB' at localhost:10703...
# OK: START TRANSACTION
# {
#   "name": "Mercury Records",
#   "location": {"city": "New York City", "state": "New York", "country": "United States of America"},
#   "foundingdate": "1945-01-01T00:00:00+00:00",
#   "countryoforigin": "United States of America",
#   "wikipediapageid": 325909,
#   "timestamp": "2023-12-23T05:56:00+00:00",
#   "id": "c6ace604-ad04-11ee-8e02-dfd1c4664cb4"
# }
# {
#   "artistname": "Geddy Lee",
#   "birthdate": null,
#   "othernames": null,
#   "genres": [{"id": "73639556-ace1-11ee-8cf5-d3a2f4f16760"}],
#   "labels": [],
#   "familyname": null,
#   "wikipediapageid": 12964,
#   "timestamp": "2024-01-03T09:44:00+00:00",
#   "id": "736419b8-ace1-11ee-8cf5-efb9dbcea8dd"
# }
# {
#   "name": "Progressive Rock",
#   "wikipediapageid": 51503,
#   "timestamp": "2023-12-26T17:13:00+00:00",
#   "id": "73639556-ace1-11ee-8cf5-d3a2f4f16760"
# }
# {
#   "title": "Rush (band)",
#   "currentmembers": [{"id": "736419b8-ace1-11ee-8cf5-efb9dbcea8dd"}],
#   "pastmembers": [],
#   "genres": [{"id": "73639556-ace1-11ee-8cf5-d3a2f4f16760"}],
#   "labels": [{"id": "c6ace604-ad04-11ee-8e02-dfd1c4664cb4"}],
#   "wikipediapageid": 124802,
#   "wikipediapagetimestamp": "2024-01-05T23:30:00+00:00",
#   "id": "6f6e3858-ad17-11ee-b3b0-3fd2dffd248e"
# }
# OK: COMMIT TRANSACTION
# Sun, Jan  7, 2024  5:19:57 PM